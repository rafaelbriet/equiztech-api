        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 bg-white">
            ...
            <?php else : ?>
                <div class="d-flex justify-content-between flex-wrap flex-md-no-wrap align-items-center border-bottom pt-3 pb-2 mb-3">
                    <h1 class="h2">Deletar categoria</h1>
                </div>

                <div class="mb-3">
                    <p>Você realmente deseja deletar a categoria "<?php echo $response['categoria']['nome']; ?>"?</p>
                    <p>Esta ação não pode ser desfeita.</p>

                    <form class="form-dashboard">
                        <div class="mb-3">
                            <input type="hidden" class="form-control" id="categoryId" value="<?php echo $response['categoria']['id']; ?>">
                            <p class="form-text text-danger d-none" id="categoryName-duplicate">Não foi possível deletar a categoria no momento. Tente novamente.</p>
                        </div>
                ...
                <script src="<?php echo $_ENV['BASE_URL']; ?>/dashboard/categorias/delete.js"></script>
            <?php endif; ?>
        </main>
    </div>
</div>
